...
function Header() {
  ...
  return (
    <div className='w-full sticky top-0 z-50'>
      {/* Image start here */}
      <div className='w-full bg-amazon_light text-white px-4  py-3 flex items-center gap-4'>
        <Link to={'/'}>
        ...
        <div className='headerHover hidden mdl:inline-flex'>
          ...
          <p className='text-sm text-lightText font-light flex flex-col'>Deliver to
            <span className='text-sm font-semibold -mt-1 text-whiteText'>Oman</span>
          </p>
        </div>
        {/* deliver end here */}
        {/* search start here */}
        <div className='h-10 rounded-md hidden  lgl:flex flex-grow relative'>
          <span onClick={() => setShowAll(!showAll)} className='w-14 h-full bg-gray-200 hover:bg-gray-300 border-2 cursor-pointer duration-300 text-sm text-amazon_light font-titleFont flex items-center justify-center rounded-tl-md rounded-bl-md'>
            All <span></span>
                ...
                <ul className='absolute w-56 h-80 top-10 left-0 overflow-y-scroll overflow-x-hidden bg-white border-[1px] border-amazon_light text-black p-2 flex-col gap-1 z-50'>
                  {
                    allItems.map((item) => (
                      <li
                        className='text-sm tracking-wide font-titleFont border-b-[1px] border-b-transparent hover:border-b-amazon_light cursor-pointer duration-200'
                        key={item._id}>
                        {item.title}
                      </li>
                    ))
